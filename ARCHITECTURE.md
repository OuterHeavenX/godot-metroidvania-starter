...
Expected malformed-file diagnostics are deliberately produced by the corruption test. Some headless runs report audio-stream shutdown warnings; the native rendering smoke test exits cleanly. Automated boss combat includes tactical teleports, so it is regression evidence rather than proof of human difficulty balance. A rendered native arena screenshot is checked; keyboard/touch playtesting remains valuable after this refactor.
