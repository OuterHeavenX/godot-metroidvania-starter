...
func run() -> void:
	...
	check(level.encounters.enemies.size() == expected_enemies, "death retains kills")
	...
	check(level.encounters.enemies.size() == expected_enemies, "Continue and death use the same surviving encounter state")
	...
	check(level.encounters.boss == null and not level.encounters.goal.locked,
		"an area with no boss leaves its exit open")
	level.elapsed = 43.5
	level._sync_clock()
...
func _boss_persistence() -> void:
	...
	var player := level.player
	player.set_physics_process(false)
	check(level.data.validation_errors().is_empty(), "the arena's authored IDs validate")
	check(level.encounters.boss != null and level.encounters.goal.locked,
		"a boss locks its exit until it falls")
	level.encounters.boss.squash()
	check(not level.encounters.boss.guarded, "the guard breaks")
	player.kill()
	await get_tree().create_timer(1.1, true, false, true).timeout
	check(level.encounters.boss.guarded, "death resets the surviving boss guard")
	level.encounters.boss.die()
	...
	SaveMan.load_progress()
	SaveMan.resume_requested = true
	level = load(BOSS_LEVEL).instantiate()
	add_child(level)
	level.set_physics_process(false)
	level.player.set_physics_process(false)
	check(level.encounters.boss == null and not level.encounters.goal.locked,
		"defeated boss and unlocked exit survive reload")
	remove_child(level)
	level.queue_free()
	await get_tree().process_frame
	SaveMan.clear_progress()
