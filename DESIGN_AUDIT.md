...
A four-area action-platformer with metroidvania ability gates. Run, variable-height jump, coyote time, wall slide/jump and an invulnerable dash are available immediately. Double jump opens traversal; ground pound breaks stone and dispatches nearby enemies. Checkpoints heal, hearts restore health, and death returns the player to their last spawn while leaving the current area's world state intact. Thrown daggers, granted by an orb in area three, answer anything out of sword reach. The route runs from the cemetery and its Undercroft into the Castle Hall, then the Armoury where the daggers are found, and out onto the Ramparts above the town. The Warden is built and tested but no longer placed in a shipped level; tests/fixtures/warden_arena.tres is where it lives. Progress and best times persist through SaveMan.
