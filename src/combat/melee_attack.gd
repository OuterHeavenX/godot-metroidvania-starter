...
## Horizontal reach, not a radius -- see resolve(). Owners set their own; the
## player's comes from MVPlayer.ATTACK_RANGE.
var reach := 86.0
...
func resolve(source: Node2D, facing: float) -> void:
	...
	for foe in nearby(source, source.global_position, reach, collision_mask):
		...
		# absf(offset.x), not offset.length(). An enemy's origin is the centre of
		# its collision box, so a tall enemy carries its origin further above the
		# floor and a radius silently shortens the swing against it: the Warden
		# sits 39px up, which turned a reach of 86 into 77 -- less than its own,
		# so it outranged the player. Enemies test their reach horizontally and
		# this now matches.
		if _hits.has(id) or absf(offset.y) >= half_height or offset.x * facing <= -back_grace or absf(offset.x) >= reach:
			continue
		_hits[id] = true
